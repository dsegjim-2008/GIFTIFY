...
router.post('/playlists', upload.single('image'), (req, res) => {
    ...
    db.query('SELECT id FROM users WHERE spotify_id = ?', [spotifyId], (err, users) => {
        ...
        let photo_url = 'https://via.placeholder.com/150/6B1FB5/FFFFFF?text=Playlist';
        if (req.file) photo_url = `http://127.0.0.1:3001/uploads/${req.file.filename}`;
...
router.post('/redeem', (req, res) => {
    ...
    db.query('SELECT id, points FROM users WHERE spotify_id = ?', [spotifyId], (err, users) => {
        if (err || users.length === 0) return res.status(404).send();
        const user = users[0];
        if (user.points < cost) return res.status(400).json({ error: "Puntos insuficientes" });
        db.query('UPDATE users SET points = points - ? WHERE id = ?', [cost, user.id], (err2) => {
            if (err2) return res.status(500).json({ error: err2.message });
            db.query('INSERT INTO redemptions (user_id, reward_id, points_spent) VALUES (?, ?, ?)', 
                [user.id, rewardId, cost], (err3) => {
                if (err3) return res.status(500).json({ error: err3.message });
                res.json({ success: true, newPoints: user.points - cost });
            });
        });
    });
});
